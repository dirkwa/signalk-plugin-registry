...
exports.pickDisplayVersion = pickDisplayVersion;
...
function pickDisplayVersion(versions) {
    ...
    const byVersionDesc = (a, b) => b[0].localeCompare(a[0], undefined, { numeric: true });
    const stable = candidates.filter(([v]) => !(0, npm_name_1.isPrerelease)(v)).sort(byVersionDesc);
...
async function main() {
    ...
    const badgeTitles = {
        'holds-back-core-deps': 'A declared dependency range pins a core Signal K package below its latest same-major release (-80)'
    };
    function esc(s) {
