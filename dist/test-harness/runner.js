    ...
    try {
        ...
        try {
            ...
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                console.error(`[runner] npm install failed, tests not runnable: ${msg.slice(0, 200)}`);
                return { hasTests: true, pass: false, runnable: false };
            }
        }
        ...
        if (buildScript) {
            ...
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                console.error(`[runner] Build failed, tests not runnable: ${msg.slice(0, 200)}`);
                return { hasTests: true, pass: false, runnable: false };
            }
        }
...
async function runPluginTest(pluginName, pluginVersion) {
    ...
    if (ownTests.hasTests && !ownTests.runnable) {
        console.error("[runner] Tests not runnable from npm package, trying source repo...");
